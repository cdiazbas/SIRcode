...
# ====================================================================
def readSIRMap(resultado, magnitud, logTau):
    from numpy import load
    try:
        resultadoSir = load(resultado)
    except:
        resultadoSir = load(resultado,encoding='latin1')
    cont = 0
    tau = tauIndex(resultadoSir, logTau)
    heightMap = resultadoSir[-1][-1][0][0]+1
    widthMap = resultadoSir[-1][-1][0][1]+1
    from numpy import zeros
    mapa = zeros((widthMap, heightMap))
    for fila in range(0, heightMap):
        for columna in range(0, widthMap):
            ...
            if magnitud == 8 or magnitud == 9 or magnitud == 10 or magnitud == 11:
                ...
                mapa[columna,fila] = resultadoSir[veces][punto][1][0][magnitud][tau]
            cont += 1
    return mapa.T

# ====================================================================
def tauIndex(resultadoSir, logTau):
    from numpy import where
    return where(resultadoSir[0][0][1][0][0] == logTau)[0][0]

# ====================================================================
def taulist(resultado):
    from numpy import load
    resultadoSir = load(resultado)
    return resultadoSir[0][0][1][0][0]
...
# ====================================================================
def readSIRProfileMap(resultadoSir, Nstoke):
    cont = 0
    [height, width, rangoLambda] = shapeSIRMap(resultadoSir)
    from numpy import zeros
    synMapa = zeros((width, height, rangoLambda))
    for fila in range(0, height):
        for columna in range(0, width):
            punto = cont % resultadoSir.shape[1]
            veces = int(cont/resultadoSir.shape[1])
            synMapa[columna, fila, :] = resultadoSir[veces][punto][2][1][Nstoke][:]
            cont += 1
    return synMapa
