C OPAC_CONTINUO evalua la opacidad la opacidad del continuo a todas las ldo a una T y Pe dadas
...
c        dpe0=1.e-6
c        dt0=2500.d0
c        open(2,file='fich_2500_22500.datos')
 
	 print*,'dame T y pe'
	 read*,dt,pe
c        do k=1,151    !1,151 indices en pe  desde 10^-6 hasta 10^9 paso .1
         
c           dpe=dpe0*10**((k-1)*.1)
c          pe=dpe
c           print*,k
c          do j=1,201 ! 1,201 T desde 2500 a 22500 con paso 100
c                           do j=1,4
c              dt=dt0+(j-1)*100.
c                            dt=5040./(0.5+j*0.3)
              t=dt
		...
		print*,lambda*1.e8,dkappa
c                suma=suma+(bt*pasol*1.d-5)/dkappa
             end do
c            kross=(4.*sigma*(dt**3.)/pi)/suma
c	     coef=sqrt(1./mui + 1./mun)
c             write(2,*) dt,dpe,dpg,dro,pesomedio,kross,pn,coef
