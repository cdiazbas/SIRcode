...
c ierror=3 no existen los modelos 1 ni 2
	...
	subroutine leemodi22(iesc,model1,model2,atmos,ntau,ierror)

	include 'PARAMETER'  !por kt
	integer iesc,icanal,ntau,ierror,nciclos,nohaycampo1,nohaycampo2
	real*4 atmos(*),a(8)
...
c            ahora leemos el modelo 1
	     ...
	     do i=1,ntau
	        read(ican,*,err=999)(atmos(i+j*ntau),j=0,7)
	     end do
	     close(ican)
           end if
...
c ahora leemos los dos modelos
	   ...
	   do i=1,ntau
	      read(ican,*,err=999)(atmos(i+j*ntau+2),j=8,15)
	   end do
	   close(ican)
...
c escribimos los modelos
	   ...
	   do i=1,ntau
	      write(ican,100)(atmos(i+j*ntau),j=0,7)
	   end do
	   close(ican)
...
989	   if(ierror.eq.0)then
	      ...
	      do i=1,ntau
	         write(ican,100)(atmos(i+j*ntau+2),j=8,15)
	      end do
	      close(ican)
	   end if
        end if 
